 ...
 * @author jose
 */
public class UsersFunctions extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        UsersDao userdao=new UsersDao();
            ...
            for (int i = 0; i < dtos.size(); i++) {
                out.println("<tr>");
                out.println("<td>" + dtos.get(i).getName() + "</td>");
                out.println("<td>" + dtos.get(i).getIdProfile() + "</td>");
                out.println("<td>" + dtos.get(i).getEmail() + "</td>");
                out.println("<td>" + dtos.get(i).getArea() + "</td>");
                out.println("<td>" + dtos.get(i).getUser() + "</td>");
                out.println("<td>" + dtos.get(i).getPass() + "</td>");
                out.println("<td>");
                out.println(" <a class=\"btn btn-info btn-xs\" data-toggle=\"modal\" data-target=\"#modal1\" ><i class=\"fa fa-pencil\"></i> Editar </a>");
                out.println(" <a class=\"btn btn-danger btn-xs\" data-toggle=\"modal\" data-target=\"#confirmacion\" onclick=\"setId("+dtos.get(i).getIdUser()+")\" ></i> Eliminar </a>");
                ...
                out.println("<li><a id=\"cerrar\" data-toggle=\"modal\" data-target=\"#cerrar\" onclick=\"setOpcion('Cerrada');setId("+dtos.get(i).getIdUser()+");changeStatus()\">Cerrar</a></li>");
                out.println("<li><a id=\"suspender\" data-toggle=\"modal\" data-target=\"#suspender\" onclick=\"setOpcion('Suspendida');setId("+dtos.get(i).getIdUser()+");changeStatus()\">Suspender</a></li>");
                out.println("<li><a id=\"reabrir\" data-toggle=\"modal\" data-target=\"#reabrir\" onclick=\"setOpcion('Reabierta');setId("+dtos.get(i).getIdUser()+");changeStatus()\">Reabir</a></li>");
                out.println("</ul>");
                out.println("</div>");                                                  
                out.println("</td>");   
                out.println("</tr>");
